...
const PopularRepositories: React.FC = () => {
  ...
  const {
    texts: { popularRepositories: repoDetails },
  } = profileConfig;
  console.log(repositories);
  return (
    <div className="mb-6">
      <h2 className="text-sm text-[#1f2328] font-semibold mb-4">
        {repoDetails.title}
      </h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {repositories.map((repo, index) => (
          <div
            className="p-4 border border-[#d1d9e0] rounded-md hover:border-[#30363d] transition-colors"
            key={repo.name + index}
          >
            <div className="flex items-center justify-between mb-2">
              ...
              <span className="text-[#59636e] text-xs border border-[#d1d9e0] rounded-full flex-shrink-0 ml-2 py-.5 px-2">
                {repo.visibility === "public"
                  ? repoDetails.publicLabel
                  : repoDetails.privateLabel}
              </span>
            </div>
